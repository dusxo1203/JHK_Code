...
int main()
{
	...
	Sample a(10), b(20), c(30);  // 시작값 0으로 설정
	a.prn();
	b.prn();
	c.prn();

	a.setNum(10);//객체명을 통한 static 멤버함수 호출
	b.setNum(10);
	Sample::setNum(10);	//동일한 효과
	a.prn();
	b.prn();
	c.prn();

	static Sample stob(40);//storage class(기억클래스)가 static인 객체이다.
